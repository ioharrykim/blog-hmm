...
- **Database:** 운영에서는 로컬 SQLite를 쓰지 않고 Neon Free Postgres를 연결합니다. 앱은 Vercel 서버리스 환경에서 TCP 타임아웃을 줄이기 위해 `@neondatabase/serverless` HTTP 드라이버를 사용합니다.
...
3. 현재 앱은 Neon HTTP 드라이버를 사용하므로 pooled/direct 둘 다 시작용으로 사용할 수 있습니다. pooled URL의 host에는 `-pooler`가 들어갑니다.
...
4. pooled URL이 계속 타임아웃되면 초기에는 direct URL, 즉 host에 `-pooler`가 없는 connection string으로 바꿔 테스트합니다. 현재 앱은 HTTP 드라이버를 사용하므로 일반 TCP 5432 연결보다 Vercel 서버리스에서 안정적입니다.
